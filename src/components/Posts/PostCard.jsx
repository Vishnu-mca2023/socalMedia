...
const PostCard = ({ post, onDelete, onLike }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-100 rounded-xl shadow hover:shadow-lg p-5 mb-6 transition-all">
      {/* Header */}
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-3">
          {post.author?.avatar ? (
            <img
              src={`https://backend-api-b0q2.onrender.com${post.author.avatar}`}
              alt="avatar"
              className="w-10 h-10 rounded-full object-cover"
            />
          ) : (
            <div className="w-10 h-10 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center font-bold text-white">
          ...
          <div className="grid gap-3 mt-3">
            {post.media.map((item, idx) => (
              <div key={idx}>
                {item.type === 'image' ? (
                  <img
                    src={`https://backend-api-b0q2.onrender.com${item.url}`}
                    alt="Post media"
      ...
      <div className="flex items-center gap-5 mb-4">
        <button
          onClick={handleLike}
          className="flex items-center gap-1 text-sm px-2 py-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700"
        ...
        <div className="mt-4 space-y-4">
          {comments.map((comment, i) => (
            <div key={i} className="flex items-start gap-3">
              {comment.author?.avatar ? (
                <img
                  src={`https://backend-api-b0q2.onrender.com${comment.author.avatar}`}
                  alt="commenter"
              ...
              <div className="bg-gray-100 dark:bg-gray-700 rounded-md px-3 py-2 w-full">
          ...
          <form onSubmit={handleAddComment} className="flex gap-3 mt-2">
            <input
              ...
              disabled={loading}
              className="flex-1 px-3 py-2 text-sm border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-900 dark:text-white dark:border-gray-600"
            />
            <button
              type="submit"
